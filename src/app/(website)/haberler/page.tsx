...
export const metadata: Metadata = {
    ...
    description: 'Ertip Medikal hakkındaki en güncel haberler, uluslararası fuarlar, etkinlikler ve sektörel duyurular.',
    openGraph: {
        ...
        <div className="container mx-auto px-4 py-16 md:py-24 min-h-screen">
            ...
            <div className="max-w-3xl mx-auto text-center mb-12 space-y-4">
                ...
                <p className="text-lg text-slate-600">
                    Ertip Medikal'in katıldığı uluslararası fuarlar, eğitimler ve en güncel duyurular.
                </p>
            </div>
          ...
          <span className="text-sm font-semibold text-slate-500 mr-2 flex items-center gap-2">
            ...
            {news.length === 0 ? (
                <div className="text-center py-24 bg-slate-50 rounded-3xl border border-dashed border-slate-300">
                    <p className="text-xl text-slate-500 font-medium">Bu kriterlere uygun bir haber bulunamadı.</p>
                    <Link href="/haberler" className="text-primary hover:underline mt-2 inline-block">Filtreleri Temizle</Link>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {news.map((item) => {
                        const imageUrl = typeof item.image === 'object' && item.image?.url ? item.image.url : '/placeholder.jpg'
                        const formattedDate = new Date(item.publishedDate).toLocaleDateString('tr-TR', {
                            year: 'numeric', month: 'long', day: 'numeric'
                        })
                        const categoryTitle = typeof item.category === 'object' ? item.category?.title : null

                        return (
                            <Card key={item.id} className="group overflow-hidden rounded-[2rem] border-0 bg-white shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
