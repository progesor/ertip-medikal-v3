...
// --- SEO VE YARDIMCI FONKSİYONLAR (Senin Kodun - Değişmedi) ---
...
export default async function ProductDetailPage({ params }: Args) {
    ...
    const breadcrumbItems = [
        ...
        ...(primaryCategory ? [{
            label: typeof primaryCategory === 'object' ? primaryCategory.title : 'Kategori',
    ...
    if (relatedProducts.length === 0 && product.category && product.category.length > 0) {
        const catId = typeof product.category[0] === 'object' ? product.category[0].id : product.category[0]
                ...
                <section className="border-t border-slate-100 mt-16 pt-20 bg-slate-50/50">
                    <div className="container mx-auto px-4 max-w-7xl">
                        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-10 gap-4">
                            <div>
                                <h2 className="text-3xl font-black text-slate-900 mb-2">İlginizi Çekebilir</h2>
                                <p className="text-slate-500">Bu ürünle benzer özelliklere sahip diğer medikal çözümlerimiz.</p>
                            </div>
                            <Button variant="outline" className="rounded-xl font-bold shrink-0" asChild>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            {relatedProducts.map((rel: any) => {
                                const relImageUrl = typeof rel.mainImage === 'object' && rel.mainImage?.url ? rel.mainImage.url : '/placeholder.jpg'

                                return (
                                    <Link key={rel.id} href={`/urunler/${rel.slug}`} className="group bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:border-primary/20 transition-all duration-300 flex flex-col">
                                        ...
                                        <h4 className="font-bold text-slate-900 line-clamp-1 mb-2 group-hover:text-primary transition-colors">{rel.title}</h4>
                                        <p className="text-xs text-slate-400 line-clamp-2 leading-relaxed mb-4 flex-1">{rel.shortDescription}</p>
                                        <div className="text-primary text-xs font-bold flex items-center gap-1 uppercase tracking-wider mt-auto">
                                            Detaylı İncele <ChevronRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
                                        </div>
                                    </Link>
                                )
                            })}
