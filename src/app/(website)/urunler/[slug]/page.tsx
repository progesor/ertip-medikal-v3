...
export default async function ProductDetailPage({ params }: Args) {
  ...
  return (
    <main className="bg-white min-h-screen pb-24">
      ...
      {relatedProducts.length > 0 && (
        <section className="border-t border-surface-muted mt-16 pt-20 bg-surface-subtle">
          <div className="container mx-auto px-4 max-w-7xl">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-10 gap-4">
              <div>
                <h2 className="text-3xl font-black text-content-strong mb-2">
                  İlginizi Çekebilir
                </h2>
              ...
              {relatedProducts.map((rel: any) => {
                ...
                return (
                  <Link
                    ...
                    className="group bg-white rounded-4xl p-6 border border-surface-muted shadow-sm hover:shadow-xl hover:border-primary/20 transition-all duration-300 flex flex-col"
                  >
                    <div className="relative aspect-square mb-6 bg-surface rounded-2xl p-4 flex items-center justify-center overflow-hidden">
                      <Image
                        src={relImageUrl}
                        alt={rel.title}
                        fill
                    ...
                    <h4 className="font-bold text-content-strong line-clamp-1 mb-2 group-hover:text-primary transition-colors">
                      {rel.title}
                    </h4>
                    <p className="text-xs text-content-subtle line-clamp-2 leading-relaxed mb-4 flex-1">
