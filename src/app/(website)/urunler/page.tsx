...
import { Search, ChevronRight, LayoutGrid, CornerDownRight, X, PackageX, ChevronLeft } from 'lucide-react'
            ...
            <div className="bg-slate-900 py-16 mb-12 border-b-4 border-primary">
                <div className="container mx-auto px-4 max-w-7xl text-center">
                    ...
                    <p className="text-lg text-slate-300 max-w-2xl mx-auto">
                        İhtiyacınız olan medikal cihazı, modeli veya SKU kodunu hızlıca bulun.
                    </p>
                </div>
            </div>
                    ...
                    <aside className="w-full lg:w-1/4 space-y-8">
                        ...
                        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                            ...
                            <form action="/urunler" method="GET" className="relative">
                                ...
                                />
                                <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 text-white bg-primary p-1.5 rounded-lg hover:bg-primary/90 transition-colors">
                                                ...
                                                <div className="ml-5 mt-1.5 mb-3 space-y-1 border-l-2 border-slate-100 pl-3">
                                                    {children.map((child: any) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <CornerDownRight className={`w-3.5 h-3.5 ${isCurrentChild ? 'text-primary' : 'opacity-40'}`} />
                                                                {child.title}
                                                            </Link>
                                                        )
                                                    })}
                    ...
                    <main className="w-full lg:w-3/4">
                        ...
                        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                            <p className="text-sm text-slate-500 font-medium">
                                Toplam <strong className="text-slate-900 text-base">{totalDocs}</strong> ürün bulundu. (Sayfa {currentPage}/{totalPages || 1})
                            </p>
                            ...
                            {(q || categorySlug) && (
                                <div className="flex flex-wrap gap-2">
                                    {q && (
                                        <Link href={`/urunler${categorySlug ? `?category=${categorySlug}` : ''}`} className="inline-flex items-center gap-1.5 bg-slate-100 hover:bg-red-50 text-slate-700 hover:text-red-600 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors">
                                            Arama: "{q}" <X className="w-3.5 h-3.5" />
                                        </Link>
                                    )}
                                    {categorySlug && activeCategory && (
                                        <Link href={`/urunler${q ? `?q=${q}` : ''}`} className="inline-flex items-center gap-1.5 bg-primary/10 hover:bg-red-50 text-primary hover:text-red-600 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors">
                                            Kategori: {activeCategory.title} <X className="w-3.5 h-3.5" />
                                        </Link>
                                    )}
                                    <Link href="/urunler" className="inline-flex items-center gap-1.5 text-slate-400 hover:text-slate-900 px-2 py-1.5 text-xs font-bold transition-colors ml-2 underline">
                                        Tümünü Temizle
                                    </Link>
                                </div>
                            )}
                        ...
                        {products.length === 0 ? (
                            <div className="bg-white p-16 rounded-[2rem] border border-slate-200 shadow-sm text-center flex flex-col items-center justify-center min-h-[400px]">
                                ...
                                <p className="text-slate-500 mb-8 max-w-md">
                                    Aradığınız kriterlere uygun ürün veya SKU kodu sistemimizde bulunmuyor. Farklı kelimelerle aramayı deneyebilirsiniz.
                                </p>
                            ...
                            <>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                                    {products.map((product: any) => {
                                        ...
                                        return (
                                            <Card key={product.id} className="group overflow-hidden rounded-[2rem] border-slate-200 shadow-sm hover:shadow-xl hover:border-primary/20 transition-all duration-300 flex flex-col bg-white">
                                                <Link href={`/urunler/${product.slug}`} className="relative aspect-square bg-slate-50 p-6 flex items-center justify-center overflow-hidden">
                                                    <Image
                                                        src={imageUrl}
                                                        alt={product.title}
                                                        fill
                                                ...
                                                <CardHeader className="pt-6 pb-2">
                                                    <CardTitle className="text-lg line-clamp-2 leading-snug font-bold">
                                                        <Link href={`/urunler/${product.slug}`} className="hover:text-primary transition-colors text-slate-900">
                                                            {product.title}
                                                        </Link>
                                                    </CardTitle>
                                                </CardHeader>
                                                <CardContent className="flex-1 pb-4">
                                                    <p className="text-sm text-slate-500 line-clamp-2 leading-relaxed">{product.shortDescription}</p>
                                                </CardContent>
                                                <CardFooter className="pt-0 pb-6 px-6">
                                                    <Button className="w-full rounded-xl font-bold bg-slate-900 hover:bg-primary transition-colors" asChild>
