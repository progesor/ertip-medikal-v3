...
export async function Footer() {
  ...
  return (
    <footer className="bg-text-main text-text-muted pt-20 pb-10 border-t border-border">
      <div className="container mx-auto px-4">
        {/* Sütun sayısına göre kendini ayarlayan akıllı grid */}
        <div
          className={`grid grid-cols-1 md:grid-cols-2 ${gridClass} gap-12 mb-16`}
        >
          {footer?.columns?.map((column: any, index: number) => {
            switch (column.blockType) {
              case "textColumn":
                ...
                const footerLogo =
                  general?.whiteLogo?.url || general?.siteLogo?.url;
                return (
        ...
        <div className="mt-16 pt-8 border-t border-border">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8 text-sm text-text-muted">
              ...
              <div className="flex items-center gap-6">
                {settings.socialMedia.map((social: any, idx: number) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-text-muted hover:text-white transition-all duration-300 font-medium flex items-center gap-2 group"
                  >
