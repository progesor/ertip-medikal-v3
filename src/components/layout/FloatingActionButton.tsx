...
export async function FloatingActionButton() {
    ...
    // Eski showPulse alanını artık "hafif kıpırdama efekti" gibi kullanıyoruz.
    ...
    const alignClass =
        position === "bottom-left" ? "items-start text-left" : "items-end text-right";
            ...
            >
              ...
              <span>{label}</span>
                  ...
                  Mesaj göndermek için tıklayın
                    ...
                    {appearance === "icon-only" && <span className="sr-only">{label}</span>}
                </Link>
            </div>
        </>
    );
