  ...
  return (
    <section className="py-24 bg-white overflow-hidden">
      <div className="container mx-auto px-4 max-w-7xl">
        <h2 className="text-3xl md:text-4xl font-extrabold text-center mb-20 text-content-strong">
          {title}
        </h2>
          ...
          {steps?.map((step: any, i: number) => (
            <React.Fragment key={i}>
              <div className="flex-1 text-center group relative z-10 w-full max-w-xs">
                ...
                <h4 className="text-xl font-bold text-content-strong mb-4">
                  {step.title}
                </h4>
              ...
              {showArrows && i < steps.length - 1 && (
                <div className="flex items-center justify-center py-4 lg:py-0 lg:pt-6">
                  {/* Masaüstü Ok (Sağa) */}
                  <ChevronRight className="hidden lg:block w-8 h-8 text-content-soft animate-pulse" />
                  {/* Mobil Ok (Aşağı) */}
                  <ChevronDown className="lg:hidden w-8 h-8 text-content-soft animate-pulse" />
                </div>
              )}
            </React.Fragment>
          ))}
