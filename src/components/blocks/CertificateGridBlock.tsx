...
export function CertificateGridBlock({
  ...
  const showNext = (event: React.MouseEvent) => {
    event.stopPropagation();
    setSelectedCertIndex((previous) =>
      previous !== null && previous < certificates.length - 1
        ? previous + 1
        : 0,
    );
  };
          ...
          {certificates.map((certificate, index) => (
            ...
            >
              <div className="relative w-full aspect-[1/1.414] bg-border/50 p-6 flex items-center justify-center overflow-hidden border-b border-border">
                ...
                <Image
                  src={certificate.image?.url || "/placeholder.jpg"}
            ...
            <div className="w-full lg:w-1/3 p-8 md:p-14 flex flex-col bg-surface overflow-y-auto">
              ...
              <div className="pt-10 mt-10 border-t border-border">
                {currentCert.document?.url ? (
                  <div className="space-y-4">
                    <Button
                      className="w-full h-16 rounded-2xl text-lg font-bold bg-surface-inverse text-surface-inverse-foreground hover:bg-primary hover:text-primary-foreground transition-all shadow-xl hover:shadow-primary/20"
                      asChild
                    >
                      <a
                        href={currentCert.document.url}
                        target="_blank"
