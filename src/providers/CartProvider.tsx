...
export function CartProvider({ children }: { children: React.ReactNode }) {
    ...
    const [toast, setToast] = useState<{ show: boolean; item: CartItem | null }>({ show: false, item: null })
                ...
                {toast.show && toast.item && (
                    ...
                    >
                        <div className="w-16 h-16 relative bg-slate-50 rounded-2xl p-1 shrink-0 border border-slate-100">
                            <Image src={toast.item.image || '/placeholder.jpg'} alt="Ürün" fill className="object-contain" unoptimized />
                        </div>
                        <div className="flex-1">
                            ...
                            <p className="text-sm font-bold text-slate-900 line-clamp-1">{toast.item.title}</p>
                            <p className="text-xs text-slate-500 font-mono mt-0.5">{toast.item.sku}</p>
                        </div>
                        <Link
                            href="/teklif-sepeti"
