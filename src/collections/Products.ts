...
export const Products: CollectionConfig = {
    ...
    hooks: {
        beforeChange: [
            ({ data }) => {
                if (data.triggerVariantGeneration && data.attributes && data.attributes.length > 0) {
                    ...
                    const parsedAttributes = data.attributes
                        ...
                        .map((attr: any) => ({
                            name: attr.name,
                            values: attr.values.split('-').map((v: string) => v.trim()).filter(Boolean)
                        }))
                    ...
                    const arraysToCombine = parsedAttributes.map((attr: any) => attr.values)
                    ...
                    const generatedVariants = combinations.map(combo => {
                        ...
                        const finalSuffix = data.skuSuffix ? ` ${data.skuSuffix.trim()}` : ''
                    ...
                    fields: [
                        ...
                        { name: 'shortDescription', type: 'textarea', label: 'Kısa Özet', maxLength: 300 },
                        {
                            ...
                            admin: {
                                ...
                                language: 'markdown',
                                description: 'Buraya Markdown (## Başlık, | Tablo |) veya doğrudan HTML (<p><strong>...</strong></p>) yapıştırabilirsiniz.'
                            }
                        },
                        // DİNAMİK TEMEL ÖZELLİKLER (YENİ EKLENDİ)
                        {
                            ...
                            fields: [
                                { name: 'key', type: 'text', label: 'Özellik (Örn: Malzeme)', required: true, admin: { width: '50%' } },
                                { name: 'value', type: 'text', label: 'Değer (Örn: Paslanmaz Çelik)', required: true, admin: { width: '50%' } }
                            ]
                        }
                    ]
                },
                    ...
                    fields: [
                        {
                            type: 'row',
                            fields: [
                                { name: 'skuPrefix', type: 'text', label: 'SKU Öneki (Örn: 110-)', admin: { width: '50%' } },
                                { name: 'skuSuffix', type: 'text', label: 'SKU Soneki (Örn: S-303)', admin: { width: '50%' } },
                            ]
                        },
                        {
                            name: 'attributes',
                            ...
                            admin: { description: 'Değerleri TİRE (-) ile ayırarak yazın. Örn: 0.6-0.65-0.7' },
                            fields: [
                                { name: 'name', type: 'text', label: 'Özellik Adı (Örn: Çap)', required: true },
                                { name: 'values', type: 'textarea', label: 'Değerler', required: true }
                            ]
                        },
                        {
                            ...
                            admin: { description: 'Bunu işaretleyip kaydettiğinizde, yukarıdaki özelliklerin tüm kombinasyonları hesaplanır, özel kurallara göre SKU kodları oluşturulur ve aşağıdaki listeye otomatik eklenir.' }
                        },
                        {
                            name: 'variants',
                            type: 'array',
                            label: 'Üretilen Varyantlar',
                            admin: { description: 'Bu liste otomatik dolar ancak sonrasında manuel müdahale edip istisnai durumları düzeltebilirsiniz.' },
                            fields: [
                                { name: 'title', type: 'text', label: 'Varyant Adı', required: true },
                                { name: 'sku', type: 'text', label: 'Varyant SKU (Barkod)', required: true },
                                { name: 'price', type: 'text', label: 'Liste Fiyatı (Opsiyonel)' },
                                { name: 'isActive', type: 'checkbox', label: 'Aktif / Satışta', defaultValue: true }
                            ]
                        }
                    ]
                },
                    ...
                    fields: [
                        {
                            ...
                            options: [
                                ...
                                { label: 'Yan Panelde (Sidebar - Özet Kartlar)', value: 'sidebar' },
                                { label: 'Her İki Yerde de Göster', value: 'both' },
                            ],
                            admin: {
                                description: 'Teknik ve lojistik verilerin ürün sayfasındaki konumunu belirler.'
                            }
                        },
                        {
                            type: 'row',
                            fields: [
                                { name: 'width', type: 'text', label: 'Net Genişlik (mm)', admin: { width: '25%' } },
                                { name: 'height', type: 'text', label: 'Net Yükseklik (mm)', admin: { width: '25%' } },
                                { name: 'depth', type: 'text', label: 'Net Derinlik (mm)', admin: { width: '25%' } },
                                { name: 'weight', type: 'text', label: 'Net Ağırlık (gr)', admin: { width: '25%' } },
                            ]
                        },
                        {
                            name: 'packaging',
                            ...
                            admin: {
                                description: 'Ürünün farklı paketleme formlarını (Örn: Tekli Kutu, 50’li Ana Koli) buraya ekleyebilirsiniz.'
                            },
                            fields: [
                                {
                                    type: 'row',
                                    fields: [
                                        { name: 'packageLabel', type: 'text', label: 'Paket Tipi (Örn: Master Carton)', required: true, admin: { width: '40%' } },
                                        { name: 'quantity', type: 'number', label: 'İçindeki Adet', admin: { width: '20%' } },
                                        { name: 'grossWeight', type: 'text', label: 'Brüt Ağırlık (kg)', admin: { width: '40%' } },
                                    ]
                                },
                                {
                                    type: 'row',
